...
│   └── ChatPage.tsx            # 主界面（侧栏 + 对话区 + 输入区，约 900 行）
├── components/
│   ├── AuthGuard.tsx           # 路由守卫
│   ├── SettingsPanel.tsx       # 设置面板（页面路由：Memory / Features / Debug / Logout）
│   ├── MemoryPanel.tsx         # 记忆管理（增删改查 + 分层过滤 + 横向标签滚动）
│   ├── FeaturesPanel.tsx       # 功能开关面板（memory_enabled 等 4 个开关）
│   └── DebugPanel.tsx          # Context Debug 面板（查看注入给 AI 的上下文）
├── stores/
│   ├── authStore.ts            # Token 持久化 + 登录状态
│   ├── sessionStore.ts         # 会话列表 + 当前会话 + 时间分组
│   └── chatStore.ts            # 消息列表 + SSE 流式解析 + 自动命名 + 错误/重试
└── api/
    ├── client.ts               # fetch 封装（自动 Bearer token，401 事件）
    ├── auth.ts                 # POST /auth/login
    ├── sessions.ts             # Session CRUD
    ├── chat.ts                 # 消息历史 + SSE 流式对话 + 消息删除 API
    └── memories.ts             # Memory CRUD
```

**已实现功能**：
- JWT 登录 + Token 持久化 + 全局 401 自动登出
- 会话 CRUD + 按时间分组 + 长按重命名（空白输入框，placeholder 显示原标题）+ 自动命名（前 20 字）
...
- Markdown + 代码高亮
- 记忆面板（三层过滤 + 增删改查）
- 设置面板（Memory / Features / Debug 子页面 + Logout）
- AI 主动搜索记忆：MemoryRefBlock 折叠显示检索结果
- **手机端完整支持**：
  - 侧边栏抽屉（hamburger + 右滑开 / 左滑关 / 遮罩点击关）
  - iOS 键盘适配（visualViewport API + keyboardOffset）
  - 所有面板 iOS safe-area padding
  - PWA manifest + Apple meta tags
- **气泡输入框**：未聚焦胶囊（rounded-full），聚焦展开（rounded-2xl），textarea 自动增高
- **消息内联操作**：每条消息下方 Copy（复制成功变 Check）+ Trash2（删除整轮）
- **错误/重试**：SSE 30s 超时 + 发送失败捕获 → 红色提示 + 重试按钮
...
include_router 挂载：
  sessions_router  → /api/sessions 系列
    ...
    DELETE /api/sessions/{id}/messages/{conv_id}  → 删除单轮对话（用户+AI一起删）✅ 新增
...
| 问题 | 说明 |
|------|------|
| ~~两套 FEATURE_FLAGS~~ | ✅ 已修复：main.py 改为 from config import FEATURE_FLAGS，统一单一数据源 |
...
| ~~OpenRouter thinking 暴露~~ | ✅ 已修复：adapters.py 新增 _adapt_openai_xml() 处理 XML 标签格式 |
...
- **后端消息格式**：GET /sessions/{id}/messages 返回的每条记录是 `{ id, user_msg, assistant_msg }` 一对，不是标准 role/content 格式；`id` 是 `conversations` 表主键，对应前端 `conversationId`
...
- **Git Tags 是回退机制**——前端每个里程碑打 tag（当前最新 `v3.6-mobile-bugfix-3`），`git checkout <tag名>` 可回退
- **iOS safe-area 规则**：
  - 顶部内容区：`paddingTop: calc(16px + env(safe-area-inset-top))`
  - 底部输入区：`paddingBottom: env(safe-area-inset-bottom)` 或 keyboardOffset
  - sidebar 底部：也需要 `paddingBottom: env(safe-area-inset-bottom)` + 显式 background
  - `body` 背景色必须与主内容区一致（`#fafbfd`），否则 iOS home indicator 横条会漏出不同颜色
- **iOS 高度单位**：用 `height: 100dvh`（动态视口高度）而非 `100vh` 或 Tailwind `h-screen`，避免 Safari 地址栏导致的页面高度错误
- **iOS 重命名 input 不要用 autoSelect**：重命名 input 启动时 value 为空，placeholder 显示原标题，autoFocus 到空字段不会触发 iOS 全选
- **消息删除数据结构**：conversations 表每行对应一个 user+AI 来回，删除时前端传 `conversationId`（= 数据库行 id），后端一次删掉整行（用户消息和 AI 回复同时删除）
- **SSE 事件 content 字段**：chatStore 的 SSE 解析中 event.content 对应 text_delta；tool_result 事件的 content 字段是记忆检索文本
