...
const MemoryRefBlock = memo(function MemoryRefBlock({ query, found, content, elapsed }: { query: string; found: number; content: string; elapsed?: number | null }) {
  ...
  return (
    <div className="mb-3" style={{ padding: '8px 14px', display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8, borderRadius: 20, border: '1px dashed rgba(196,154,120,0.25)', background: 'rgba(196,154,120,0.04)', cursor: 'pointer', maxWidth: '100%', minWidth: 0 }} onClick={() => setOpen(o => !o)}>
      {isActive ? <span className="tool-spinner" /> : <Search size={11} strokeWidth={1.8} style={{ color: C.memoryRefAccent, opacity: 0.8, flexShrink: 0 }} />}
  ...
  return (
    <div className="mb-3" style={{ display: 'flex', flexDirection: 'column', gap: 4, padding: '8px 14px', borderRadius: 12, border: '1px dashed rgba(196,154,120,0.2)', background: 'rgba(196,154,120,0.03)', cursor: 'pointer', maxWidth: '100%', minWidth: 0 }} onClick={() => setOpen(o => !o)}>
      <div className="flex items-center gap-2" style={{ color: C.textSecondary }}>
        <BookOpen size={11} strokeWidth={1.8} style={{ color: C.memoryOpsAccent, opacity: 0.8, flexShrink: 0 }} />
...
const DevToolOpsBlock = memo(function DevToolOpsBlock({ ops }: { ops: DevToolOp[] }) {
  ...
  return (
    ...
    >
      <div className="flex items-center gap-2" style={{ color: C.textSecondary }}>
        <Wrench size={11} strokeWidth={1.8} style={{ color: C.toolsAccent, opacity: 0.85, flexShrink: 0 }} />
        <span style={{ fontSize: 11, fontFamily: ROOM_FONT }}>
          Tools · {ops.length}
        </span>
        {open ? <ChevronDown size={10} strokeWidth={2} style={{ color: C.textMuted }} /> : <ChevronRight size={10} strokeWidth={2} style={{ color: C.textMuted }} />}
...
const ThinkingBlock = memo(function ThinkingBlock({ text, thinkingTime }: { text: string; thinkingTime?: number | null }) {
  ...
  return (
    <div className="mb-3 cursor-pointer" onClick={() => setOpen(o => !o)}
      style={{
        padding: '12px 18px',
        borderLeft: '2px solid rgba(139,130,148,0.3)',
      ...
      onMouseEnter={e => (e.currentTarget.style.borderLeftColor = 'rgba(139,130,148,0.55)')}
      onMouseLeave={e => (e.currentTarget.style.borderLeftColor = 'rgba(139,130,148,0.3)')}
    >
      <div className="flex items-center gap-2" style={{ fontStyle: 'normal', fontSize: 10.5, letterSpacing: '0.12em', textTransform: 'uppercase' as const, color: C.textMuted, marginBottom: open ? 8 : 0 }}>
        <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="rgba(139,130,148,0.7)" strokeWidth="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
