...
import { X, Copy, Check, Download, Code, Eye, Edit3, History, ChevronLeft, ChevronRight } from 'lucide-react'
...
// ─── Writing Mode (Markdown split editor) ────────────────────────────────────
...
function WritingMode({ content: initialContent }: { content: string }) {
  ...
  return (
    <div className="flex h-full">
      <div className="w-1/2 border-r overflow-auto" style={{ borderColor: '#e8ecf5' }}>
        <CodeMirrorView content={content} language="markdown" editable onChange={setContent} />
      </div>
      <div className="w-1/2 overflow-auto p-6 md-content" style={{ background: '#fff' }}>
        <ReactMarkdown rehypePlugins={[rehypeHighlight]}>{content}</ReactMarkdown>
      </div>
    </div>
  )
...
function VersionNavigator() {
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2 flex-shrink-0" style={{ borderBottom: '1px solid #e8ecf5', background: 'rgba(0,47,167,0.03)' }}>
      <History size={13} style={{ color: '#7a8399' }} />
      <span className="text-xs font-medium" style={{ color: '#5a6a8a' }}>v{currentVersion?.version || currentIdx + 1} / {total} 个版本</span>
      <div className="flex items-center gap-1 ml-auto">
        <button onClick={() => viewVersion(currentIdx + 1)} disabled={currentIdx >= total - 1} className="flex items-center justify-center rounded p-1 cursor-pointer disabled:opacity-30" style={{ color: '#7a8399' }}>
          <ChevronLeft size={14} />
        </button>
        <span className="text-xs tabular-nums" style={{ color: '#9aa3b8', minWidth: 40, textAlign: 'center' }}>{total - currentIdx} / {total}</span>
        <button onClick={() => viewVersion(currentIdx - 1)} disabled={currentIdx <= 0} className="flex items-center justify-center rounded p-1 cursor-pointer disabled:opacity-30" style={{ color: '#7a8399' }}>
...
export default function ArtifactPanel() {
  ...
  return (
    <div
      ...
      style={{ width: undefined, minWidth: 0, borderLeft: '1px solid #e8ecf5', background: '#fafbfd', flexShrink: 0 }}
    >
      <style>{`@media (min-width: 768px) { .animate-slide-in-right { width: 500px !important; max-width: 50vw !important; } }`}</style>
      ...
      <div className="flex items-center gap-3 px-4 py-3 flex-shrink-0" style={{ borderBottom: '1px solid #e8ecf5' }}>
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-sm font-medium truncate" style={{ color: '#1a1f2e' }}>{currentArtifact.title}</h3>
          <p className="text-xs" style={{ color: '#9aa3b8' }}>
            {currentArtifact.language && `${currentArtifact.language} · `}{currentArtifact.type}
            {currentArtifact.version > 1 && ` · v${currentArtifact.version}`}
          </p>
        </div>
        <div className="flex items-center gap-1.5">
          <button onClick={handleCopy} className="flex items-center justify-center rounded-lg p-1.5 cursor-pointer" style={{ color: copied ? '#22c55e' : '#7a8399' }} title="复制">
            {copied ? <Check size={15} /> : <Copy size={15} />}
          </button>
          <button onClick={handleDownload} className="flex items-center justify-center rounded-lg p-1.5 cursor-pointer" style={{ color: '#7a8399' }} onMouseEnter={e => (e.currentTarget.style.color = '#002FA7')} onMouseLeave={e => (e.currentTarget.style.color = '#7a8399')} title="下载">
            <Download size={15} />
          </button>
          <button onClick={closePanel} className="flex items-center justify-center rounded-lg p-1.5 cursor-pointer" style={{ color: '#7a8399' }} onMouseEnter={e => (e.currentTarget.style.color = '#ef4444')} onMouseLeave={e => (e.currentTarget.style.color = '#7a8399')} title="关闭">
            <X size={15} />
          </button>
        </div>
      </div>
      ...
      <div className="flex flex-shrink-0" style={{ borderBottom: '1px solid #e8ecf5' }}>
        <button
          ...
          className="flex items-center gap-1.5 px-4 py-2.5 text-xs font-medium cursor-pointer"
          style={{ color: activeTab === 'code' ? '#002FA7' : '#7a8399', borderBottom: activeTab === 'code' ? '2px solid #002FA7' : '2px solid transparent' }}
        >
          <Code size={13} /> 代码
        </button>
        {canPreview && (
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2.5 text-xs font-medium cursor-pointer"
            style={{ color: activeTab === 'preview' ? '#002FA7' : '#7a8399', borderBottom: activeTab === 'preview' ? '2px solid #002FA7' : '2px solid transparent' }}
          >
            <Eye size={13} /> 预览
          </button>
        )}
        {isMarkdown && (
          <button
            ...
            className="flex items-center gap-1.5 px-4 py-2.5 text-xs font-medium cursor-pointer"
            style={{ color: activeTab === 'writing' ? '#002FA7' : '#7a8399', borderBottom: activeTab === 'writing' ? '2px solid #002FA7' : '2px solid transparent' }}
          >
            <Edit3 size={13} /> 写作间
          </button>
        )}
